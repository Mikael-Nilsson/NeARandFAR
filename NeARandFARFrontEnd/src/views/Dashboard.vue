<template>
  <div>
    <template v-if="this.private.activeConversation">
      <template
        v-for="(reply, index) in this.private.activeConversation.replies"
        :key="index"
      >
        <button v-on:click="updateConversation(reply)">
          {{ reply.line }}
        </button>
      </template>
    </template>

    <button id="showmap" v-on:click="toggleMap()">
      {{ this.private.gotoView }}
    </button>
    <button id="showScene" v-on:click="shared.follow = !shared.follow">
      position: {{ shared.follow ? "on" : "off" }}
    </button>
...
import { globalState } from "../components/VuexAvoidanceStore";
